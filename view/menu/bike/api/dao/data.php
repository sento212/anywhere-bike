<?php
function data_all($sqlbuilder, $_no_regis, $_unit_no)
{
    ...
    if ($dat['reg_approval_web'] != '' or $dat['reg_approval_web_02'] != '' or $dat['reg_approval_web_03'] != '' or $dat['reg_approval_web_04'] != '') {
        $url_api = "https://www.araksa.com/prog-x/api/anywhere_bike/doc_appr";
        $data_api = array("reg_approval_web1" => "$dat[reg_approval_web]", "reg_approval_web2" => "$dat[reg_approval_web_02]", "reg_approval_web3" => "$dat[reg_approval_web_03]", "reg_approval_web4" => "$dat[reg_approval_web_04]");
        $header = array('Content-Type: multipart/form-data', 'id: mks', 'key: robbymahorobbymahorobbymahorobbymaho');
        $doc_appr = $sqlbuilder->api('POST', $url_api, $data_api, $header);
        if ($doc_appr['status'] == 0) {
            return $doc_appr;
        }
        $data['data'][0]['doc_appr'] = $doc_appr['data'];
    }
    if (($dat['register_id'] != '') and ($dat['survey_no'] != '')) {
        ...
        Std_panel,
            ...
            panel_roles,
        panel_roles_sa
        from polis_agent_direct_hasil_survey
        where     register_id = '$dat[register_id]'
        ...
        $data['data'][0]['hasil_surv'] = $hasil_surv['data'];
    } else {
        $data['data'][0]['hasil_surv'] = [];
    }
    $result = [
        "status" => 200,
